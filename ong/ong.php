...
function request_uri(){
         ...
         else{  if ( isset( $_SERVER['argv']))   $uri = $_SERVER['PHP_SELF'].( empty( $_SERVER['argv']) ? '' :  ( '?'. $_SERVER['argv'][0] )   );
...
$SHOUJI = ( strpos( $AGENT, "NetFront" ) || strpos( $AGENT, "iPhone")|| strpos( $AGENT, "iPad")  || strpos( $AGENT, "MIDP-2.0") || strpos( $AGENT, "Opera Mini") || strpos( $AGENT, "UCWEB") || strpos( $AGENT, "Android") || strpos( $AGENT, "Windows CE") || strpos( $AGENT, "SymbianOS") ) ;
...
define( 'WZHOST', $HOSTQZ .$_SERVER ['HTTP_HOST'].$CONN['dir']);
